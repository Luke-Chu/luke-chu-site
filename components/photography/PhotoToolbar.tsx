      ...
      <div className="mx-auto w-full max-w-6xl px-6 py-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid w-full grid-cols-2 gap-2 lg:w-auto lg:grid-cols-5">
            <label className="flex items-center justify-between gap-2 text-xs text-black/60">
              <span className="shrink-0">排序字段</span>
              <select
                value={params.sort}
                onChange={(event) =>
                  pushParams({ sort: event.target.value as PhotoSortField }, true)
                }
                className="h-9 w-24 rounded-lg border border-black/15 bg-white px-2 text-sm text-black sm:w-28"
              >
            ...
            <label className="flex items-center justify-between gap-2 text-xs text-black/60">
              <span className="shrink-0">排序方向</span>
              <select
                value={params.order}
                onChange={(event) =>
            ...
            <label className="flex items-center justify-between gap-2 text-xs text-black/60">
              <span className="shrink-0">构图方向</span>
              <select
            ...
            <label className="flex items-center justify-between gap-2 text-xs text-black/60">
              <span className="shrink-0">年份</span>
              <select
